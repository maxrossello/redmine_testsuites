...
class Redmine::WikiFormatting::MacrosTest < Redmine::HelperTest
  ...
  def test_issue_macro_should_render_link_to_issue
    ...
    if Redmine::Plugin.installed?(:redwine)
      # priority given not per id (which cannot be controlled), but per position
      assert_equal %{<p><a class="issue tracker-1 status-1 priority-1 priority-lowest" href="/issues/1">Bug #1</a>: #{issue.subject}</p>}, textilizable("{{issue(1)}}")
      assert_equal %{<p>eCookbook - <a class="issue tracker-1 status-1 priority-1 priority-lowest" href="/issues/1">Bug #1</a>: #{issue.subject}</p>}, textilizable("{{issue(1, project=true)}}")
    else
